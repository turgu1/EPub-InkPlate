...
//
  ...
  #define U16(v) static_cast<uint16_t>(v)
    ...
    void getEventTask(void *param) {
      ...
      //   LOG_I("Got: {}, [{}, {}] [{}, {}]", count, x[0], y[0], x[1], y[1]);
      // }

      while (true) {
        event = { .kind = EventMgr::EventKind::NONE, .x = 0, .y = 0, .dist = 0 };

        LOG_D("State: {}", uint8_t(state));
        ...
        case State::PINCHING:
          if (xQueueReceive(touchscreenIsrQueue, &ioNum, maxDelayMs / portTICK_PERIOD_MS)) {
            ...
            if (count == 0) {
              ...
              LOG_D("Distance diffs: {} {}", lastDist, newDistDiff);
              if (newDistDiff != 0) {
        ...
        if (event.kind != EventMgr::EventKind::NONE) {
          LOG_D("Input Event {} [{}, {}] ({})...", EventMgr::eventStr[int(event.kind)], event.x,
                event.y, event.dist);
    ...
    auto EventMgr::toUserCoord(uint16_t &x, uint16_t &y) -> void {
      ...
      if (a == 0) {
        ...
        LOG_D("Coordinates to transform: [{}, {}]", x, y);
    ...
    auto EventMgr::calibrationEvent(const Event &event) -> bool {
      ...
      case EventKind::TAP:
        ...
        if (calibCount >= 3) {
          ...
          LOG_D("Touch points: [{}, {}] [{}, {}] [{}, {}]", X0,  Y0,  X1,  Y1,  X2,  Y2);
          LOG_D("Calib points: [{}, {}] [{}, {}] [{}, {}]", XD0, YD0, XD1, YD1, XD2, YD2);
          ...
          LOG_D("Coefficients: a:{} b:{} c:{} d:{} e:{} f:{} divider:{}", a, b, c, d, e,
                f, divider);
    ...
    -> gboolean {
      ...
      LOG_D("State: {}", std::static_cast<uint8_t>(state));
      ...
      if (event.kind != EventMgr::EventKind::NONE) {
        LOG_D("Input Event {} [{}, {}] ({})...", EventMgr::eventStr[int(event.kind)], event.x, event.y,
              event.dist);
      ...
      while (1) {
        ...
        if (event.kind != EventKind::NONE) {
          ...
          if (!stayOn) { // Unless somebody wants to keep us awake...
            ...
            LOG_D("Light Sleep for {} minutes...", lightSleepDuration);
            ESP::delay(500);
