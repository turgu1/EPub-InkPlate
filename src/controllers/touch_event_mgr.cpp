...
//
  ...
  const char *EventMgr::event_str[9] = {"NONE",         "TAP",         "HOLD",
  ...
  #define U16(v) static_cast<uint16_t>(v)
    ...
    static void IRAM_ATTR touchscreen_isr_handler(void *arg) {
    ...
    void get_event_task(void *param) {
      ...
      enum class State : uint8_t {
        NONE, WAIT_NEXT, HOLDING, SWIPING, PINCHING
      };
      ...
      uint16_t x_start = 0, y_start = 0, x_end = 0, y_end = 0, last_dist = 0;
      ...
      while (true) {
        ...
        case State::PINCHING:
          if (xQueueReceive(touchscreen_isr_queue, &io_num, MAX_DELAY_MS / portTICK_PERIOD_MS)) {
            ...
            if (count == 0) {
              ...
              if (new_dist_diff != 0) {
                event.kind = (last_dist < this_dist) ? EventMgr::EventKind::PINCH_ENLARGE
        ...
        if (event.kind != EventMgr::EventKind::NONE) {
          LOG_D("Input Event %s [%u, %u] (%u)...", EventMgr::event_str[int(event.kind)], event.x,
                event.y, event.dist);
    ...
    void EventMgr::show_calibration() {
      ...
      case Screen::Orientation::BOTTOM:
        LOG_D("Bottom...");
        calib_point = {.x = {U16(Screen::get_width() - 100), U16(Screen::get_width() / 2), 100},
                       .y = {100, U16(Screen::get_height() - 100), U16(Screen::get_height() / 2)}};
        break;

      case Screen::Orientation::TOP:
        LOG_D("Top...");
        calib_point = {.x = {100, U16(Screen::get_width() / 2), U16(Screen::get_width() - 100)},
                       .y = {U16(Screen::get_height() - 100), 100, U16(Screen::get_height() / 2)}};
        break;

      case Screen::Orientation::LEFT:
        LOG_D("Left...");
        calib_point = {.x = {U16(Screen::get_width() - 100), 100, U16(Screen::get_width() / 2)},
                       .y = {U16(Screen::get_height() - 100), U16(Screen::get_height() / 2), 100}};
        break;

      case Screen::Orientation::RIGHT:
        LOG_D("Right...");
        calib_point = {.x = {100, U16(Screen::get_width() - 100), U16(Screen::get_width() / 2)},
    ...
    void EventMgr::to_user_coord(uint16_t &x, uint16_t &y) {
      ...
      if (a == 0) {
        ...
        case Screen::Orientation::BOTTOM:
          ...
          x    = Screen::get_width() -
                 ((uint32_t)(Screen::get_width() - 1) * temp) / touch_screen.get_y_resolution();
          break;
        ...
        case Screen::Orientation::TOP:
          ...
          temp = y;
          y    = Screen::get_height() -
                 ((uint32_t)(Screen::get_height() - 1) * x) / touch_screen.get_x_resolution();
        ...
        case Screen::Orientation::LEFT:
          LOG_D("Left...");
          x = Screen::get_width() -
              ((uint32_t)(Screen::get_width() - 1) * x) / touch_screen.get_x_resolution();
          y = Screen::get_height() -
              ((uint32_t)(Screen::get_height() - 1) * y) / touch_screen.get_y_resolution();
          break;
    ...
    bool EventMgr::calibration_event(const Event &event) {
      ...
      case EventKind::TAP:
        ...
        if (calib_count >= 3) {
          ...
          #define SWAP(i0, i1)                                                                     \
            {                                                                                      \
              temp              = touch_point.x[i0];                                               \
              touch_point.x[i0] = touch_point.x[i1];                                               \
              touch_point.x[i1] = temp;                                                            \
              temp              = touch_point.y[i0];                                               \
              touch_point.y[i0] = touch_point.y[i1];                                               \
              touch_point.y[i1] = temp;                                                            \
            }
          ...
          calib_point = {.x = {100, U16(e_ink.get_height() - 100), U16(e_ink.get_height() / 2)},
          ...
          LOG_D("Coefficients: a:%lld b:%lld c:%lld d:%lld e:%lld f:%lld divider:%lld", a, b, c, d,
                e, f, divider);
    ...
    void EventMgr::retrieve_calibration_values() {
    ...
    static gboolean mouse_event_callback(GtkWidget *event_box, GdkEvent *gdk_event, gpointer data) {
      #define DISTANCE (sqrt(pow(x_end - x_start, 2) + pow(y_end - y_start, 2)))
      ...
      static uint16_t x_start = 0, y_start = 0, x_end = 0, y_end = 0, last_dist = 0;
