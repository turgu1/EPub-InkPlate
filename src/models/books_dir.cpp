...
const BooksDir::EBookRecord *BooksDir::get_book_data(uint16_t idx) {
...
const BooksDir::EBookRecord *BooksDir::get_book_data_from_db_index(uint16_t idx) {
        ...
        if (temp_index.find(fname) == temp_index.end()) {
          ...
          if (epub.open_file(fname)) {
            ...
            if (!filename.empty()) {
              ...
              auto img = epub.get_image(filename, true);
              if (img == nullptr) {
                LOG_D("Unable to retrieve cover file: %s", filename.c_str());
                memcpy(the_book->cover_bitmap, default_cover,
