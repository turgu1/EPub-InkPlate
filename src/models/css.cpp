...
CSS::PropertyMap CSS::property_map = {{"not-used", CSS::PropertyId::NOT_USED},
...
CSS::FontSizeMap CSS::font_size_map = {{"xx-small", 6}, {"x-small", 7},  {"smaller", 9},
...
CSS::CSS(const char *css_id, const char *file_folder_path, const char *buffer, int32_t size,
         uint8_t prio) {
  id          = css_id;
...
CSS::CSS(const char *css_id, DOM::Tag tag, const char *buffer, int32_t size, uint8_t prio) {
  id          = css_id;
  folder_path = "";
  ghost       = false;
  priority    = prio;
...
bool CSS::match_simple_selector(DOM::Node &node, SelectorNode &simple_sel) {
  ...
  if ((simple_sel.tag != DOM::Tag::NONE) && (simple_sel.tag != DOM::Tag::ANY) &&
      (simple_sel.tag != node.tag))
    return false;
...
bool CSS::match_selector(DOM::Node *node, Selector &sel) {
  SelOp op            = SelOp::NONE;
  DOM::Node *the_node = node;
  // The selector_node_list is already in a reverse order, so we process selectors from right to
  // left
...
void CSS::match(DOM::Node *node, RulesMap &to_rules) {
  for (auto &rule : rules_map) {
...
void CSS::show(RulesMap &the_rules_map) {
  #if DEBUGGING
