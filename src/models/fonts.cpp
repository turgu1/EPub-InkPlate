...
static constexpr const char *font_fnames[8] = {"CrimsonPro",  "Caladea",
                                               "Asap",        "AsapCondensed",
                                               "DejaVuSerif", "DejaVuSerifCondensed",
                                               "DejaVuSans",  "DejaVuSansCondensed"};

static constexpr const char *font_labels[8] = {"CRIMSON S", "CALADEA S",  "ASAP",
...
char *Fonts::get_file(const char *filename, uint32_t size) {
...
bool Fonts::setup() {
  ...
  if ((stat(xml_fonts_descr, &file_stat) != -1) &&
    ...
    // System fonts
    auto sys_group = fd.child("fonts").find_child_by_attribute("group", "name", "SYSTEM");
...
void Fonts::adjust_default_font(uint8_t font_index) {
...
void Fonts::clear_glyph_caches() {
  for (auto &entry : font_cache) {
    entry.font->clear_cache();
  }
}

int16_t Fonts::get_index(const std::string &name, FaceStyle style) {
  int16_t idx = 0;
...
bool Fonts::replace(int16_t index, const std::string &name, FaceStyle style,
  ...
  if ((f.font = FontFactory::create(filename))) {
    if (f.font->is_ready()) {
      ...
      LOG_D("Font %s (%s) replacement at index %d and style %d.", f.name.c_str(), filename.c_str(),
            f.font->get_fonts_cache_index(), (int)f.style);
      return true;
    } else {
      delete f.font;
    }
...
bool Fonts::add(const std::string &name, FaceStyle style, const std::string &filename) {
  std::scoped_lock guard(mutex);
...
bool Fonts::add(const std::string &name, FaceStyle style, MemoryFontPtr buffer, int32_t size,
                const std::string &filename) {
  std::scoped_lock guard(mutex);
...
Fonts::FaceStyle Fonts::adjust_font_style(FaceStyle style, FaceStyle font_style,
                                          FaceStyle font_weight) const {
