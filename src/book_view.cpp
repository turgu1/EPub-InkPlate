...
int16_t
BookView::get_pixel_value(const CSS::Value & value, const Page::Format & fmt, int16_t ref)
{
...
int16_t
BookView::get_point_value(const CSS::Value & value, const Page::Format & fmt, int16_t ref)
{
  switch (value.value_type) {
    case CSS::PX:
...
float
BookView::get_factor_value(const CSS::Value & value, const Page::Format & fmt, float ref)
{
  switch (value.value_type) {
    case CSS::PX:
    case CSS::PT:
...
void
BookView::adjust_format_from_suite(Page::Format & fmt, const CSS::PropertySuite & suite)
{  
...
bool 
BookView::get_image(std::string & filename, Page::Image & image)
{
...
BookView::build_page_recurse(xml_node<> * node, Page::Format fmt)
{
  ...
  if (*(name = node->name())) { 
    ...
    if (started) compute_mode = DISPLAY;
    ...
    if (current_offset + (size = strlen(str)) <= start_of_page_offset) {
      ...
      if (current_offset >= start_of_paragraph) compute_mode = DISPLAY;
      while (*str) {
        if (uint8_t(*str) <= ' ') {
          if (current_offset >= start_of_page_offset) {
            if (*str == ' ') {
...
void
BookView::build_page_at(const EPub::Location & loc)
{
  bool done = false;
  TTF * font = fonts.get(0, 10);
  page_bottom = font->get_line_height() + (font->get_line_height() >> 1);
