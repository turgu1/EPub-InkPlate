## Fonts Licensing

All fonts used by this application have been optimized to fit the device's storage capacity while retaining the required code points for all 16 supported languages.

Below is the licensing information for each font family. For each license used, we have provided the corresponding font families and the links where you can download them.

### Sil Open Font Version 1.1

See the license text [Here](./licenses/Sil-Open-Font-Version-1.1.txt)

- [Asap](https://www.1001fonts.com/asap-font.html)
- [Bitter](https://fonts.google.com/specimen/Bitter)
- [CMUConcrete](https://font.download/font/cmu-concrete)
- [CMUSansSerif](https://font.download/font/cmu-sans-serif)
- [CMUSerif](https://font.download/font/cmu-serif)
- [CMUTypeWriter](https://font.download/font/cmu-typewriter-text)
- [CrimsonPro](https://befonts.com/crimson-pro-font-family.html)
- [Cuprum](https://font.download/font/cuprum)
- [FiraSans](https://www.1001fonts.com/fira-sans-font.html)
- [IbarraRealNova](https://fonts.google.com/specimen/Ibarra+Real+Nova)
- [RedHatDisplay](https://www.1001fonts.com/red-hat-font.html)

### Apache License, Version 2.0

See the license text [Here](./licenses/Apache-V2.0.txt)

- [Caladea](https://www.1001fonts.com/caladea-font.html)
- [OpenSans](https://www.1001fonts.com/open-sans-font.html)
- [Roboto](https://font.download/font/roboto)
- [RobotoCondensed](https://font.download/font/roboto-condensed)

### DejaVu License

See the license text [Here](./licenses/DejaVu.txt)

- [DejaVuSans](https://dejavu-fonts.github.io/)
- [DejaVuSerif](https://dejavu-fonts.github.io/)

## Interface Icon Font (drawings.otf)

The application interface utilizes an OTF icon font containing required SVG graphics converted to TTF using FontForge. These resulting font is then converted from TTF to OTF format using supplementary tools. This conversion workflow is fully automated by the `./subsetter.sh` script.

### Icons Licensing

The icon library includes mixed licensing:

* **MIT License:** Applied to specific third-party icons.
* **Apache License:** Applied to specific third-party icons.
* **Custom Designs:** Created by the author and available under either the MIT or Apache license at your discretion.

See the Apache license text [Here](./licenses/Apache-V2.0.txt)

See the MIT license text [Here](./licenses/MIT.txt)

The icons were retrieved from the following [location](https://icon-sets.iconify.design).