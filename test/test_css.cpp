...
static auto testParagraphProperties(const CSS::RulesMap &rules) -> void {
  ...
  // text-transform: uppercase → TextTransform::UPPERCASE
  {
    ...
    if (v) {
      SUITE_CHECK(v->choice.textTransform == TextTransform::UPPERCASE,
  ...
  // display: block → Display::BLOCK
  {
    ...
    if (v) {
      SUITE_CHECK(v->choice.display == Display::BLOCK, "p: display != BLOCK");
    }
  }
  // vertical-align: sub → VAlign::SUB
  {
    const CSS::Value *v = firstValue(matched, CSS::PropertyId::VERTICAL_ALIGN);
    SUITE_CHECK(v != nullptr, "p: no vertical-align value");
    if (v) {
      SUITE_CHECK(v->choice.vAlign == VAlign::SUB, "p: vertical-align != SUB");
    }
  }
}
...
static auto testClassProperties(const CSS::RulesMap &rules) -> void {
  ...
  for (const auto &[sel, props] : rules) {
    ...
    }
    {
      ...
      if (v) {
        SUITE_CHECK(v->choice.hAlign == HAlign::CENTER, ".highlight: text-align != CENTER");
      }
    }
    {
      ...
      if (v) {
        SUITE_CHECK(v->choice.display == Display::INLINE, ".highlight: display != INLINE");
      }
    }
    break;
  }
  ...
  for (const auto &[sel, props] : rules) {
    ...
    single.insert({const_cast<CSS::Selector *>(sel), props});
    {
      ...
      if (v) {
        SUITE_CHECK(v->choice.display == Display::INLINE_BLOCK,
                    "#main: display != INLINE_BLOCK");
      }
    }
    {
      const CSS::Value *v = firstValue(single, CSS::PropertyId::TEXT_ALIGN);
      ...
      if (v) {
        SUITE_CHECK(v->choice.hAlign == HAlign::RIGHT, "#main: text-align != RIGHT");
      }
    }
    {
      const CSS::Value *v = firstValue(single, CSS::PropertyId::FONT_SIZE);
  ...
  for (const auto &[sel, props] : rules) {
    ...
    for (const auto &p : *props) {
      if (p.id == CSS::PropertyId::DISPLAY) {
        ...
        if (vi != p.values.end()) {
          SUITE_CHECK(vi->choice.display == Display::NONE, ".hidden: display != NONE");
        }
      }
    }
    break;
  ...
  for (const auto &[sel, props] : rules) {
    ...
    for (const auto &p : *props) {
      if (p.id == CSS::PropertyId::TEXT_TRANSFORM) {
        auto vi = p.values.begin();
        if (vi != p.values.end()) {
          SUITE_CHECK(vi->choice.textTransform == TextTransform::NONE,
                      ".no-transform: text-transform != NONE");
        }
      }
    }
  ...
  for (const auto &[sel, props] : rules) {
    ...
    for (const auto &p : *props) {
      if (p.id == CSS::PropertyId::TEXT_TRANSFORM) {
        auto vi = p.values.begin();
        if (vi != p.values.end()) {
          SUITE_CHECK(vi->choice.textTransform == TextTransform::LOWERCASE,
                      ".lower: text-transform != LOWERCASE");
        }
      }
    }
  ...
  for (const auto &[sel, props] : rules) {
    ...
    for (const auto &p : *props) {
      if (p.id == CSS::PropertyId::TEXT_ALIGN) {
        auto vi = p.values.begin();
        if (vi != p.values.end()) {
          SUITE_CHECK(vi->choice.hAlign == HAlign::LEFT, ".left-align: text-align != LEFT");
        }
      }
    }
    break;
  ...
  for (const auto &[sel, props] : rules) {
    ...
    for (const auto &p : *props) {
      if (p.id == CSS::PropertyId::VERTICAL_ALIGN) {
        auto vi = p.values.begin();
        if (vi != p.values.end()) {
          SUITE_CHECK(vi->choice.vAlign == VAlign::SUPER,
                      "sup: vertical-align != SUPER");
        }
      }
    }
...
static auto testInlineStyle() -> void {
  ...
  auto css =
      CSS::Make("inline", DOM::Tag::SPAN, inlineCss, static_cast<int32_t>(strlen(inlineCss)), 0);
...
auto testCss() -> TestStats {
  ...
  if (!css) {
    ...
    ++css_fail;
    return TestStats{css_pass, css_fail};
  }
