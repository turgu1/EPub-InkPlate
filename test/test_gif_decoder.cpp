...
static constexpr const char *kGifResultsDir = "test/results/gif_decoder/";
...
struct DrawSink {
  ...
  int linesSeen{0};
  bool valid{true};
  bool requireSolidBlack{false};
  std::vector<uint8_t> rowSeen{};
};

auto drawCb(GifDraw *draw) -> int32_t {
  if (draw == nullptr || draw->pPixels == nullptr) return 0;

  auto *sink = static_cast<DrawSink *>(draw->pUser);
  if (sink == nullptr) return 0;

  if (draw->iBpp != 8 || draw->iHeight != 1 || draw->iWidth != sink->expectedW) {
    sink->valid = false;
    return 0;
  }

  const int row = draw->y - sink->expectedY0;
  if (row < 0 || row >= sink->expectedH) {
    sink->valid = false;
    return 0;
  }
  ...
  if (sink->requireSolidBlack) {
    for (int i = 0; i < draw->iWidth; ++i) {
      if (draw->pPixels[i] != 0x00) {
        sink->valid = false;
        return 0;
      }
    }
...
// 8x8 single-image GIF with a 2-entry global palette and all pixels at index 0.
constexpr uint8_t kGif8x8SolidBlack[] = {
    0x47, 0x49, 0x46, 0x38, 0x39, 0x61, 0x08, 0x00, 0x08, 0x00, 0x80, 0x00, 0x00, 0x00,
    0x00, 0x00, 0xff, 0xff, 0xff, 0x2c, 0x00, 0x00, 0x00, 0x00, 0x08, 0x00, 0x08, 0x00,
    0x00, 0x02, 0x25, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x00, 0x01, 0x02, 0x04, 0x08,
    0x10, 0x20, 0x40, 0x80, 0x00, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x00,
    0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x00, 0x01, 0x02, 0x05, 0x00, 0x3b};

auto runScaleCase(GifOptions opts, int expectedW, int expectedH) -> void {
  GifDecoder decoder;
  const int openOk =
      decoder.openRAM(kGif8x8SolidBlack, static_cast<int32_t>(sizeof(kGif8x8SolidBlack)), drawCb);
  CHECK(openOk == 1);
  CHECK(decoder.getLastError() == GifError::SUCCESS);
...
auto ensureGifResultsDir() -> bool {
  std::error_code ec;
  std::filesystem::create_directories(kGifResultsDir, ec);
  return !ec;
}
...
auto loadBmpAsGray(const std::string &path, int expectedW, int expectedH,
  ...
  if (!(compression == 0U || compression == 3U)) return false;
...
auto compareAgainstExpectedBitmap(const GifFixture &fixture, const std::vector<uint8_t> &actual)
    -> void {
  CHECK(actual.size() == fixture.expectedBitmap.size());
  if (actual.size() != fixture.expectedBitmap.size()) return;

  static constexpr int kPerPixelTolerance         = 18;
  static constexpr double kMaxMeanAbsDiff         = 36.0;
  static constexpr double kMaxTolMismatchRatio    = 0.24;
  static constexpr double kMaxBinaryMismatchRatio = 0.07;
...
auto drawCbCapture(GifDraw *draw) -> int32_t {
  if (draw == nullptr || draw->pPixels == nullptr) return 0;
  ...
  std::memcpy(dst, draw->pPixels, static_cast<std::size_t>(draw->iWidth));
...
auto parseFixtureLine(const std::string &rawLine, GifFixture &fixture) -> bool {
...
auto loadGifFixtures() -> std::vector<GifFixture> {
  static constexpr const char *kBaseDir  = "test/fixtures/gifs/";
...
auto decodeFixtureWithDimensionChecks(const GifFixture &fixture) -> void {
  GifDecoder decoder;
  ...
  const int decOk = decoder.decode(0, 0, GifOptions{});
  CHECK(decOk == 1);
  CHECK(decoder.getLastError() == GifError::SUCCESS);
  ...
  const std::string outPath = std::string(kGifResultsDir) + fileStem(fixture.fileName) + ".bmp";
...
auto drawCbNoop(GifDraw *draw) -> int32_t {
  if (draw == nullptr || draw->pPixels == nullptr) return 0;
  if (draw->iWidth <= 0 || draw->iHeight != 1 || draw->iBpp != 8) return 0;
  return 1;
}
...
auto testGifDecoder() -> TestStats {
  std::printf("[testGifDecoder]\n");
  checks   = 0;
  failures = 0;

  CHECK(ensureGifResultsDir());

  runScaleCase(GifOptions{}, 8, 8);

  GifOptions half{};
  half.set(GIF_OPTION(SCALE_HALF));
  runScaleCase(half, 4, 4);

  GifOptions quarter{};
  quarter.set(GIF_OPTION(SCALE_QUARTER));
  runScaleCase(quarter, 2, 2);

  GifOptions eighth{};
  eighth.set(GIF_OPTION(SCALE_EIGHTH));
  runScaleCase(eighth, 1, 1);

  const std::vector<GifFixture> fixtures = loadGifFixtures();
  for (const auto &fixture : fixtures) {
    decodeFixtureWithDimensionChecks(fixture);
  }

  if (!fixtures.empty()) {
    runMemoryChurnCheck(fixtures);
  }

  std::printf("[testGifDecoder] checks=%d failures=%d\n", checks, failures);
