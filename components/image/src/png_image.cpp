...
static void on_draw(pngle_t *pngle, uint32_t x, uint32_t y, uint8_t pix, uint8_t alpha) {
  ...
  auto data      = ((PngImage *)mypngle_get_user_data(pngle))->get_image_data();
  auto scale     = ((PngImage *)mypngle_get_user_data(pngle))->get_scale_factor();
...
PngImage::PngImage(std::string filename, Dim max, bool load_bitmap) : Image() {
  ...
  if (unzip.open_stream_file(filename.c_str(), file_size)) {
    ...
    auto size = WORK_SIZE;
    while (unzip.get_stream_data((char *)work.get(), size)) {
      if (size == 0) break;
