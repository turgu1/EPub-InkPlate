  ...
  if (fontsDB) {
    if (thisIsAppFonts) {
      ...
      for (int i = 0; i < 3; ++i) {
        ...
        if (descr) {
          LOG_D("Loading font {}...", (*descr)->name);
          if (!add(*descr)) {
            LOG_E("Unable to load font {}", (*descr)->name);
          }
        }
      }
    } else {
...
auto Fonts::replace(int16_t index, const FontFaceDescriptorPtr &descr) -> bool {
  ...
  if ((f->font = FontFactory::create(descr, library))) {
    if (f->font->isReady()) {
      ...
      LOG_D("Font {} ({}) replacement at index {} and style {}.", f->name,
            descr->filename, f->font->getFontsCacheIndex(), (int)f->style);
      return true;
    }
  } else {
...
void Fonts::adjustDefaultFont(uint8_t fontIndex) {
  ...
  if (thisIsAppFonts) {
    ...
    if (fontCache.at(3)->name.compare(fontsDB->getStandardFontName(fontIndex)) != 0) {
      ...
      LOG_D("Default font is now {}", fontsDB->getStandardFontName(fontIndex));
    }
  }
}
...
auto Fonts::add(const FontFaceDescriptorPtr &descr) -> bool {
  ...
  if ((f->font = FontFactory::create(descr, library))) {
    if (f->font->isReady()) {
      ...
      LOG_D("Font {} added to cache at index {} and style {}.", f->name,
