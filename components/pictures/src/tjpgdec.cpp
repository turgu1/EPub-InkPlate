      ...
      { // Create fast huffman decode table
        ...
        for (i = b = 0; b < HUFF_BIT; b++) { // Create LUT
          for (j = pb[b]; j; j--) {
            ...
            if (cls) {
              td = pd[i++] |
                   ((b + 1) << 8); // b15..b8: code length, b7..b0: zero run and data length
...
static JRESULT mcu_load(JDEC *jd // Pointer to the decompressor object
  ...
  for (blk = 0; blk < nby + 2; blk++) {    // Get nby Y blocks and two C blocks
    ...
    if (cmp && jd->ncomp != 3) { // Clear C blocks if not exist (monochrome image)
      ...
      tmp[0] =
          d * dqf[0] >> 8; // De-quantize, apply scale factor of Arai algorithm and descale 8 bits
      ...
      if (JD_FORMAT != 2 || !cmp) { // C components may not be processed if in grayscale output
        if (z == 1 ||
            (JD_USE_SCALE && jd->scale == 3)) { // If no AC element or scale ratio is 1/8, IDCT can
                                                // be ommited and the block is filled with DC value
...
//-----------------------------------------------------------------------
// Output an MCU: Convert YCrCb to RGB and output if in RGB form
//-----------------------------------------------------------------------
...
JRESULT jdec_prepare(JDEC *jd,                                    // Blank decompressor object
  ...
  for (;;) { // Parse JPEG segments
    ...
    case 0xDA: // SOS - Start of Scan
      ...
      jd->workbuf = alloc_pool(
          jd, len); // and it may occupy a part of following MCU working buffer for RGB output
      if (!jd->workbuf) return JDR_MEM1; // Err: not enough memory
      jd->mcubuf = (jd_yuv_t *)alloc_pool(jd, (n + 2) * 64 *
...
JRESULT jdec_decomp(JDEC *jd,                                // Initialized decompression object
                    int (*outfunc)(JDEC *, void *, JRECT *), // RGB output function
