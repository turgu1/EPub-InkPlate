# EPub-InkPlate - User's Guide - Version 1.3.2
...
To do so, a fonts configuration file named `fonts_list.xml` is used to define which font can be selected. This file, if present in the main SDCard folder, is loaded at boot time or after deep sleep to initialize the structure of the fonts. Only the first 8 fonts will be read. 
