# EPub-InkPlate - User's Guide - Version 2.0.0
...
- Book Reader - Presents the book content, one page at a time for reading.
...
Inkplate models are using different eInk screens that are using different pixel resolutions. If you ever want to transfer an SD-Card from a model to another, it could be beneficial to erase some files in the SD-Card's `books` folder.

The best way to do it is to plug the SD-Card into your computer or laptop and delete all those `.locs` and `.toc` files. The `.pars` files are the same for all Inkplate models.
