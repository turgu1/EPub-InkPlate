...
auto IBMF::getGlyphInternal(uint32_t glyphCode, int16_t glyphSize) -> Glyph * {
  ...
  if (found) {
    ...
    } else if (!face->getGlyph(glyphCode, *glyph, &glyphData,
                               glyphLoadMode_ == GlyphLoadMode::WITH_BITMAP)) {
      bitmapGlyphPool.deallocate(glyph);
      LOG_E("Unable to render glyph for glyphCode: {}", glyphCode);
      return nullptr;
    }
    ...
    getOrCreateGlyphs(currentFontSize)[glyphCode] = glyph;
    return glyph;
  }
}
    ...
    while (true) {
      if (step->tag == 1) {
        if (step->nextCharCode == nextCharcode) {
          ...
          LOG_D("Kerning between {} and {}: {}", (char)glyphData->charCode, (char)nextCharcode,
                kern);
          break;
        }
      } else {
        if (step->nextCharCode == nextCharcode) {
          LOG_D("Ligature between {} and {}", (char)glyphData->charCode, (char)nextCharcode);
