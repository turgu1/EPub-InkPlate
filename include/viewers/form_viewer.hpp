  ...
  protected:
    ...
    void compute_field_dim() {
      ...
      for (int i = 0; i < form_entry.choice_count; i++) {
        ...
        if (form_entry.choices[i].value == * (uint8_t *) entry.value) {
          item_idx = i;
        }
      }
      items.reverse();
...
class FormUInt16 : public FormField
{
  public:
    ...
    void paint(Page::Format & fmt) {
      char val[8];
      itoa(val, (uint16_t) *(form_entry.value));
    }
  protected:
    void compute_field_dim() {
...
class FormViewer
{
  ...
  public:
    ...
    std::forward_list<FormField *> fields;
    ...
    void show(FormEntries form_entries, int8_t size, const std::string & bottom_msg) {
      ...
      for (int i = 0; i < size; i++) {
        ...
        if (field != nullptr) fields.push_front(field);
      }
